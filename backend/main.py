...
# Initialize components , LLM and vector EmBeddings Begin***********************************
...
load_dotenv()
CORRECTION_ENABLED = False # False/True
max_corrections = 2
CORRECTION_LLM = LLMProvider.GROQ 
SELECTED_LLM = LLMProvider.GROQ # LLM used for initial transformation
...
def transform_new_website_chunk(body_doc, url, filename):
    ...
    chunk_info_filename = f"chunks_info_{filename}.txt"
    save_chunks_to_file(output_folder_path, final_chunks, chunk_info_filename)
    ...
    for index, chunk in enumerate(final_chunks):
        ...
        correction_needed = CORRECTION_ENABLED
        ...
        while correction_needed and correction_iteration < max_corrections:
            ...
            corrected_output_requirements_match = re.search(
                r"## MANDATORY OUTPUT RULES:\s*(.*?)(?=\n##|\Z)", 
                correction_response, 
                re.DOTALL
            )
            ...
            save_correction_prompt_response_to_file(
                output_folder_path,
                correction_chunk_filename,
            ...
            formatted_prompt = create_transform_prompt(
                corrected_output_requirements_str,
                chunk,
                summary,
                design_standard_css,
    ...
    # -------- Final HTML Assembly --------
    INLINE_CSS = True  # Toggle between inline or linked CSS
    final_body = "\n".join(f"\n{chunk}" for chunk in transformed_chunks)

    if INLINE_CSS:
        css_minified = minify_css_file(css_file_path)
        html_head = f"<style>{css_minified}</style>"
    else:
        html_head = '<link rel="stylesheet" href="poc-ds.css">'
